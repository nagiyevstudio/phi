...
const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL || "https://api.phi.nagiyev.com/index.php";
...
export const budgetApi = {
  ...
  setBudget: (month: string, plannedAmountMinor: number) =>
    api.put<{ month: string; plannedAmountMinor: number }>(
      `/months/${month}/budget`,
      {
        plannedAmountMinor,
      },
...
export const operationsApi = {
  ...
  create: (data: CreateOperationRequest) =>
    api.post<Operation>("/operations", data),
  update: (id: string, data: Partial<CreateOperationRequest>) =>
...
export const analyticsApi = {
  ...
  getYearIncome: (year: string) =>
    api.get<YearlyIncomeAnalyticsData>("/analytics", { year }),
  getIncomeYears: () =>
    api.get<IncomeYearsData>("/analytics", { years: "income" }),
  getOverallIncome: () =>
    api.get<OverallIncomeAnalyticsData>("/analytics", { overall: "income" }),
};
...
export const exportApi = {
  json: (month?: string) =>
    api.download(
      "/export/json",
      `export_${month || "all"}_${new Date().toISOString().split("T")[0]}.json`,
      { month },
    ),
  csv: (month?: string) =>
    api.download(
      "/export/csv",
      `export_${month || "all"}_${new Date().toISOString().split("T")[0]}.csv`,
      { month },
    ),
};
