...
- `public_html/api/` - если API будет доступен по адресу `ваш-домен.com/api/`
- `public_html/backend/` - если API будет доступен по адресу `ваш-домен.com/backend/`
- Или отдельный поддомен `api.ваш-домен.com` → тогда в корне этого поддомена
...
APP_ENV=production
APP_URL=https://ваш-домен.com/api
```
...
Откройте `https://ваш-домен.com/api/test.php` в браузере.

Если видите "✅ Подключение к БД успешно!" - всё работает!

**После проверки удалите файл `test.php` из соображений безопасности.**
...
```php
header("Access-Control-Allow-Origin: https://ваш-домен-frontend.com");
```
...
- Убедитесь, что хост БД правильный (может быть не `localhost`, а что-то вроде `mysql.yourdomain.com`)
...
**Примеры запросов:**
- Регистрация: `POST https://ваш-домен.com/api/auth/register`
- Вход: `POST https://ваш-домен.com/api/auth/login`
