...
- Новый способ авторизации для server-to-server: API keys.
- Старые JWT-эндпоинты для фронтенда остались без изменений.
- Внешний сервер может использовать те же маршруты `operations`, `categories`, `analytics`, `months`, `export`, но вместо JWT передавать API key.
...
- В репозитории есть миграция `database/migrations/007_create_api_keys.sql`.
- Код также умеет создать таблицу `api_keys` автоматически при первом обращении к API keys, чтобы backend не зависел от ручного запуска миграций после FTP-деплоя.
